...
function table(headers, rows) {
  ...
  return `<div class="table-wrap"><table><thead><tr>${headers.map((item) => `<th>${item}</th>`).join('')}</tr></thead><tbody>${rows
...
const RENDER = {
  overview() {
    ...
    return `
      <div class="grid cards">
        ...
        ${statCard('Comandos pendentes', number(pendingCommands), 'Fila de execucao')}
      </div>
  ...
  farm() {
    ...
    return `
      ...
      <div class="card">
        ${sectionHead('Regras', 'Configuracao do farm', adminButtons)}
        <div class="inline-stats">
      ...
      <div class="card">
        ${sectionHead('Historico', 'Snapshots do farm')}
  ...
  acoes() {
    ...
    return `
      ...
      <div class="card">
        ${sectionHead('Combate', 'Historico de acoes')}
  ...
  commands() {
    ...
    const catalogCards = catalog.map((command) => `
      <div class="command-card">
        ...
        <p class="muted">${escapeHtml(command.description || 'Sem descricao.')}</p>
        <div class="inline-stats">
    ...
    const composer = isAdmin ? `
      <div class="card">
        ${sectionHead('Execucao', 'Comando customizado')}
        <div class="grid three">
          <div class="field"><label for="cmd-type">Tipo</label><input id="cmd-type" type="text" placeholder="ex: escala.publicar"></div>
          <div class="field"><label for="cmd-label">Label</label><input id="cmd-label" type="text" placeholder="Nome visivel"></div>
    ...
    ` : `<div class="card"><p class="muted">Seu acesso e somente leitura. Historico visivel, execucao bloqueada.</p></div>`;
    ...
    return `
      ...
      <div class="card">
        ${sectionHead('Historico', 'Ultimos comandos')}
    ...
    return `
      <div class="card">
        ...
        <div class="grid three">
          ...
          <div class="field"><label for="cfg-description">Descricao</label><input id="cfg-description" type="text" value="${escapeHtml(bot.description || '')}" ${readOnly}></div>
      ...
      <div class="card">
        ...
        <div class="field"><label for="new-bot-description">Descricao</label><input id="new-bot-description" type="text" placeholder="Descricao do bot" ${readOnly}></div>
...
const APP = {
  ...
  renderShell() {
    ...
    $('bot-description').textContent = bot.description || 'Sem descricao.';
  ...
  renderPage() {
    ...
    $('page-content').innerHTML = RENDER[STATE.page]();
    this.renderBanner();
  },
  ...
  async saveSettings() {
    try {
      ...
      await this.refresh();
      this.notify('Configuracoes salvas e comando de sync enfileirado.', 'success');
    } catch (error) {
      this.notify(error.message, 'error');
    }
  },
